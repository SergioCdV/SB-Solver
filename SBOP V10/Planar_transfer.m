...
DoF = 2;
Control_var = 2;
OptProblem = Problem().DefineSolver(n, basis, m, time_distribution).AddDynamics(DoF, Control_var, L); 
...
% Boundary conditions
S0 = [1; 0; 0; 1];
SF = [1.1; 0; 0; sqrt(1/1.1)];

% Add boundary conditions
OptProblem = OptProblem.AddBoundaryConditions(S0, SF).AddParameters([mu; T]);

% Add functions 
OptProblem = OptProblem.AddFunctions(@(initial, final, beta, t0, tf)BoundaryConditionsT(initial, final, beta, t0, tf), @(params, beta, t0, tf, tau, s)ControlFunctionT(params, beta, t0, tf, tau, s), ...
                                     @(params, beta, t0, tf, s, u)CostFunctionT(params, beta, t0, tf, s, u), @(beta, P)LinConstraintsT(beta, P), ...
                                     @(params, beta, t0, tf, tau, s, u)NlinConstraintsT(params, beta, t0, tf, tau, s, u), ...
                                     @BoundsFunctionT, ...
                                     @(params, initial, final)InitialGuessT(params, initial, final));
