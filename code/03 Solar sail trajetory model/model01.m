...
%% Calculation variables to be defined for each run

m = 90; % number of discretization points
time_distribution = 'normal'; % distribution of time intervals
sigma = 1; % if normal distribution is selected

animations = 1; % set to 1 for gif
...
% .... to be completed with the solar sail model

alphan1 = deg2rad(54.7); % sail pitch angle which maximises thrust cone angle [rad] (see Huo et al 2018, [19])
ac = 0.5e-03; % characteristic acceleration, max at r0 [m/s^2]
...
[initial, final] = initial_coordinates(r0);
...
kapp = 1/3; % estimated average thrust coefficient

[tfapp, Papp, Bapp, Capp] = initial_approximation(initial, final, tau, m, mu, r0, kapp, ac, alphan1);
