...
function [B, tau] = state_basis(n, tau, basis)
    ...
    switch (basis)
        ...
        case 'Chebyshev'
            for i = 1:length(n)
                B{i} = [CH_basis('first', n(i), tau); CH_derivative('first', n(i), tau, 1)];
            end

        case 'Legendre'
             for i = 1:length(n)
                B{i} = [LG_basis(n(i), tau); LG_derivative(n(i), tau, 1)];
             end

        case 'Hermite'
             for i = 1:length(n)
                B{i} = [HT_basis(n(i), tau); HT_derivative(n(i), tau, 1)];
             end
             
        case 'Laguerre'
             for i = 1:length(n)
                B{i} = [LR_basis(n(i), tau); LR_derivative(n(i), tau, 1)];
             end
