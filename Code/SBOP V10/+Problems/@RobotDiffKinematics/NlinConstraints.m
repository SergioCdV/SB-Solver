...
function [c, ceq] = NlinConstraints(obj, params, beta, t0, tf, tau, s, u)
    ...
    c = [-reshape(s(7:9,:), 1, [])-params(3) ... % Constraint on the torque magnitude (infinity norm in epigraph form)
          reshape(s(7:9,:), 1, [])-params(3) ... % Constraint on the torque magnitude (infinity norm in epigraph form)
         -reshape(s(10:12,:), 1, [])-params(4) ... % Constraint on the torque magnitude (infinity norm in epigraph form)
          reshape(s(10:12,:), 1, [])-params(4) ... % Constraint on the torque magnitude (infinity norm in epigraph form)
          1e-6-reshape(detJ, 1, []);
        ];                                                                                      
