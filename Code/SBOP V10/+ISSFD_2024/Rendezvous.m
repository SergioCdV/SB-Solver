...
sigma = zeros(1,3);                                                             % MRP
omega = 0 * deg2rad( [0 3.53 3.53] );                                               % Angular velocity in the chaser body frame
...
y0(13:24,1) = [St(1,7:12).'; S0(7:12)];                                  % Attitude initial conditions

% Nominal trajectory 
[tspan, s_ref] = ode45(@(t,s)j2_dynamics(mu, J2, Re, zeros(3,2), 0, TOF, 0, n, COE(2), It, I, t, s), [0 TOF], y0, options);
...
%     solver.maxIter = 10;
    ...
    % Plant dynamics 
    [tspan, s] = ode45(@(t,s)j2_dynamics(mu, J2, Re, Pu(1:3,:), t0, tf, Fmax, n, params(3), It, I, t, s), [0 Ts], y0, options);  
    ...
    for i = 1:size(tspan,1)
        ...
        tau_aux = Pu(4:6,:) * PolynomialBases.Legendre().basis(size(Pu,2)-1, tau );
        if norm(tau_aux) > Tmax
            tau_aux = Tmax * tau_aux / norm(tau_aux); 
        end
    ...
    St(iter+1,7:12) = s(end,13:18);                             % Target attitude state

    S0(1:6) = s(end,7:12).'-s(end,1:6).';                       % Relative state vector in the ECI frame
    S0(7:12) = s(end,19:24).';                                  % Chaser attitude state
...
C(7:9,:) = C(7:9,:) * Vc;       % Relative velocity
...
function [dr] = j2_dynamics(mu, J2, Re, P, t0, tf, Fmax, n, e, It, Ic, t, s)
    % LVLH transformation 
    ...
    sigma_t = [s(13:15,1); -1];
    if ( norm(sigma_t) > 1 )
        sigma_t = - sigma_t / norm(sigma_t)^2;
    end

    dr(13:15,1) = 0.25 * QuaternionAlgebra.Quat2Matrix( sigma_t ) * s(16:18,1);        % Target attitude kinematics
    dr(16:18,1) = cross(It * s(16:18,1), s(16:18,1));                                  % Target attitude dynamics

    % Chaser attitude dynamics
    sigma_c = [s(13:15,1); -1];
    if ( norm(sigma_c) > 1 )
        sigma_c = - sigma_c / norm(sigma_c)^2;
    end

    dr(19:21,1) = 0.25 * QuaternionAlgebra.Quat2Matrix( sigma_c ) * s(22:24,1);        % Target attitude kinematics
    dr(22:24,1) = cross(Ic * s(22:24,1), s(22:24,1));                                  % Target attitude dynamics
end
