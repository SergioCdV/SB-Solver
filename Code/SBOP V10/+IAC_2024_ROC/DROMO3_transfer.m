...
StateDimension = 8;             % Dimension of the configuration vector. Note the difference with the state vector
...
S0(1) = 1 + initial_coe(2) * cos(S0(8));
S0(2) = 0 + initial_coe(2) * sin(S0(8));
S0(3) = 1 / (S0(3)^2 *(2*S0(1)-S0(1)^2-S0(2)^2));
...
SF(1) = 1 + final_coe(2) * cos(SF(8));
SF(2) = 0 + final_coe(2) * sin(SF(8));
SF(3) = 1 / (SF(3)^2 *(2*SF(1)-SF(1)^2-SF(2)^2));

% Spacecraft parameters 
T = 0.5e-3;              % Maximum acceleration 
T = T/gamma;             % Normalized acceleration

problem_params = [mu; T; final_coe(2); OrbitalDynamics.KeplerSolver(final_coe(2), final_coe(end))];

% Create the problem
OptProblem = IAC_2024_ROC.DROMO3(S0, SF, L, StateDimension, ControlDimension, problem_params);
...
for i = 1:length(tau)
    S(:,i) = OrbitalDynamics.dromo32state(C(1:8,i));
end
