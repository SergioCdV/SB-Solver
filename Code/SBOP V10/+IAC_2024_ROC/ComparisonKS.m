...
% Optimization
tic
[C_ecc, dV_ecc, u_ecc, t0_ecc, tf_ecc, tau_ecc, exitflag(2), output] = solver.solve(OptProblem_ecc);
toc 
...
u = u(1:3,:);
S = LegoKS.KS_mapping(C, false, "1", mu);                   % Compute the true trajectory in Cartesian space 
Gamma(1,:) = dot(C(1:4,:), C(1:4,:), 1);                    % Sundman transformation
deltaT(1) = trapz(tau, Gamma(1,:), 2);                      % Time of flight

l(1,:) = LegoKS.bilinear_function( C(1:4,:), C(5:8,:) );    % Bilinear error

% Eccentric KS
u_ecc = u_ecc ./ dot(C_ecc(1:4,:), C_ecc(1:4,:), 1).^2;             % Control vector
u_ecc = u_ecc(1:3,:);
S_ecc = LegoKS.KS_mapping(C_ecc, false, "Ecc", mu);                 % Compute the true trajectory in Cartesian space 
[~, h] = LegoKS.OscEnergy(mu, C_ecc, "Ecc");
Gamma(2,:) = dot(C_ecc(1:4,:), C_ecc(1:4,:), 1) ./ sqrt(mu .* h);   % Sundman transformation
deltaT(2) = trapz(tau_ecc, Gamma(2,:), 2);                          % Time of flight
l(2,:) = LegoKS.bilinear_function( C_ecc(1:4,:), C_ecc(5:8,:) );    % Bilinear error
