...
n = [20; 20];                           % Polynomial order in the state vector expansion
...
SF = [1/(pi+1);2];              % Final conditions

% Create the problem
OptProblem = Problems.RossTest3(S0, SF, L, StateDimension, ControlDimension);
...
figure;
hold on
scatter(tau, [(1-sin(tau)+tau).^(-1); 1-cos(tau)])
plot(tau, C(1:2,:))
xlabel('Flight time')
ylabel('$X$ coordinate')
hold off
...
scatter(tau, cos(tau)+sin(tau)-(1+tau))
xlabel('Flight time')
ylabel('$\mathbf{u}$')
grid on;